...
class NoiseGen(object):
    ...
    def save_sims(self,seed,sims,season,patch,array,mask_patch,coadd=True):
        pout,cout,sout = get_save_paths(self._model,self._version,coadd=coadd,season=season,patch=patch,array=array,overwrite=False,mask_patch=mask_patch)
        nfreqs, nsplits, npol, Ny, Nx = sims.shape

        if self._model == 'dr5':
            freqs = len(self._qid)
            print('freqs,nfreqs',freqs,nfreqs)
            assert freqs == nfreqs
            insplits = 4
        else:
        ...
        for i in range(nfreqs):
            if self._model == 'dr5':
                for j in range(nsplits):
                    fname = sout+os.path.basename(self.dm.get_map_fname(self._qid[i],j,ivar=False)).replace(".fits","_seed_%d.fits" % seed)
...
def get_n2d_data(splits,ivars,mask_a,coadd_estimator=False,flattened=False,plot_fname=None,dtype=None,dg=4):
    ...
    if flattened:
        ...
        if plot_fname is not None: plot(plot_fname+"_wmaps",data*mask_a*sivars,dg=dg)
        ...
        if plot_fname is not None: plot(plot_fname+"_fft",data*mask_a,dg=dg)
...
def generate_noise_sim(covsqrt,ivars,seed=None,dtype=None):
    ...
    del ivars
    print('kmap shape',np.shape(kmap))
    outmaps = 0.*kmap.real
    for i in range(kmap.shape[1]):
        outmaps[:,i,:,:] = enmap.ifft(kmap[:,i,:,:], normalize="phys").real
    del kmap
...
def noise_power(kmaps1,weights1,kmaps2=None,weights2=None,coadd_estimator=False,pfunc=None):
...
def get_n2d(ffts,wmaps,plot_fname=None,coadd_estimator=False,dtype=None):
    ...
    for i in range(ncomps):
        for j in range(i,ncomps):
            ...
            n2d[i,j] = noise_power(isplits,iwts,jsplits,jwts,coadd_estimator=coadd_estimator,pfunc=naive_power)
            if i!=j: n2d[j,i] = n2d[i,j] # symmetric matrix
            if plot_fname is not None: 
                plot("%s_%d_%s_%d_%s" % (plot_fname,ifreq,pols[ipol],jfreq,pols[jpol]),enmap.enmap(np.arcsinh(np.fft.fftshift(n2d[i,j])),wcs),dg=4)
...
def smooth_ps(ps,dfact=(16,16),radial_fit_lmin=300,
    ...
    # Do diagonals first
    for i in range(ncomps):
        if radial_pairs is None: 
            do_radial = True
        else: 
            do_radial = True if (i,i) in radial_pairs else False
        ...
        sps[i,i] = dnoise.copy()
        if plot_fname is not None: 
            plot("%s_unsmoothed_%d_%d" % (plot_fname,i,i), enmap.enmap(np.arcsinh(np.fft.fftshift(ps[i,i])),wcs) )
            plot("%s_smoothed_%d_%d" % (plot_fname,i,i), enmap.enmap(np.arcsinh(np.fft.fftshift(sps[i,i])),wcs) )
    ...
    for i in range(ncomps):
        for j in range(i+1,ncomps):
            if radial_pairs is None: 
                ...
                do_radial = True if (i,j) in radial_pairs or (j,i) in radial_pairs else False
            afit = ps[i,j].copy()
            if not(do_radial):
            ...
            dnoise = np.nan_to_num(dnoise * mul)

            assert not np.isnan(dnoise).any()

            sps[i,j] = dnoise.copy()
            if i!=j: sps[j,i] = dnoise.copy()
            if plot_fname is not None: 
                plot("%s_unsmoothed_%d_%d" % (plot_fname,i,j), enmap.enmap(np.arcsinh(np.fft.fftshift(ps[i,j])),wcs), dg=4 )
                plot("%s_smoothed_%d_%d" % (plot_fname,i,j), enmap.enmap(np.arcsinh(np.fft.fftshift(sps[i,j])),wcs), dg=4 )
