...
class _SeedTracker(object):
    ...
    def get_noise_seed(self, set_idx, sim_idx, data_model, season, patch, array, patch_id=None):
        ...
        if dm.name == 'dr5':
            dm_actmr3 = dmint.models['act_mr3']()
            sid =  dm_actmr3.seasons.index(season)
            if patch == 'day_14h_n':
                pid = dm_actmr3.patches.index('boss')
            else:
                pid =  dm_actmr3.patches.index(patch)
            aid = list(dm_actmr3.array_freqs.keys()).index(array)            
        else:
