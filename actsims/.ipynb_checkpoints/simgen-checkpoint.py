...
class SimGen(object):
    ...
    def get_sim(self,season,patch,array,sim_num, save_alm=True, save_map=False, set_idx=0,mask_patch=None,fgflux="15mjy"):
        ...
        if self.model == 'dr5':
            if self.qid[0] == 'boss_d03':
                afreqs = [array+'_f090',array+'_f150']
            elif self.qid[0] == 's16_d02':
                afreqs = [array+'_f090',array+'_f150']
            else:
                afreqs = [array+'_f150']
        else:
        ...
        for afreq in afreqs:
            ...
            if self.model in ['act_mr3','dr5']: 
                pfreq = afreq.split('_')[1] 
        ...
        #return self._footprint(noise.apply_ivar_window(signals + noises,ivars)), ivars
        #return self._footprint(signals+noises), self._footprint(ivars)
        return self._footprint(signals), self._footprint(noises), self._footprint(ivars)
